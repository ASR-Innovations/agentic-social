...
export default function WebhooksPage() {
  ...
  const loadWebhooks = async () => {
    try {
      const data = await get('/api/integrations/webhooks');
      setWebhooks(data);
    } catch (error) {
  ...
  const handleCreate = async () => {
    try {
      await post('/api/integrations/webhooks', newWebhook);
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      await put(`/api/integrations/webhooks/${id}/status`, { status: newStatus });
      loadWebhooks();
    } catch (error) {
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {webhooks.map((webhook) => (
            <div key={webhook.id} className="border rounded-lg p-6 hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center mb-2">
                    <h3 className="text-lg font-semibold mr-3">{webhook.name}</h3>
                    <span
              ...
              <div className="flex items-center space-x-6 text-sm text-gray-600">
                ...
                <div>
                  <span className="font-medium">Failed:</span> {webhook.failureCount}
                </div>
