...
const onboardingSteps = [
  { id: 'profile', title: 'Business Profile', description: 'Tell us about your business', icon: Building },
  { id: 'connect', title: 'Connect Platforms', description: 'Link your social media accounts', icon: LinkIcon },
  { id: 'ai-setup', title: 'AI Configuration', description: 'Customize your AI preferences', icon: Brain },
  { id: 'team', title: 'Team Setup', description: 'Invite your team members', icon: Users },
  { id: 'first-post', title: 'First Post', description: 'Create your first AI-powered post', icon: Zap },
];
...
export default function OnboardingPage() {
  ...
  const skipOnboarding = () => { router.push('/app/dashboard'); };
        ...
        <div className="w-full max-w-2xl space-y-6">
            ...
            <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                <div className="text-center space-y-2 mb-6">
                  <h1 className="text-xl md:text-2xl font-bold text-text-primary">{currentStepData.title}</h1>
                  <p className="text-sm text-text-muted">{currentStepData.description}</p>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-2xl p-8 border border-gray-100">
                  <div className="space-y-6">
                    {currentStep === 0 && (
                      <div className="space-y-6">
                        <Input label="Business Name" placeholder="e.g., Acme Corporation" />
                        <div className="space-y-6">
                          <div>
                    ...
                    {currentStep === 1 && <SocialAccountsStep />}
                              ...
                              return (
                                ...
                                >
                                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
                                    ...
                                    <LevelIcon className={`w-3.5 h-3.5 ${selectedAutomation === level.id ? 'text-white' : 'text-gray-500'}`} />
                                  </div>
                                  ...
                                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                                    selectedAutomation === level.id ? 'border-brand-green bg-brand-green' : 'border-gray-300'
                                  }`}>
                                    {selectedAutomation === level.id && <Check className="w-2.5 h-2.5 text-white" />}
                                  </div>
                                </button>
                              );
                            })}
                    ...
                    {currentStep === 3 && (
                      <div className="space-y-4">
                        <p className="text-text-muted text-xs text-center mb-4">Invite team members to collaborate on your social media strategy</p>
                        <div className="space-y-3">
                        ...
                        <div className="text-center">
                          <p className="text-text-muted text-[10px]">You can always invite team members later from settings</p>
                        </div>
                      </div>
                    )}
                        ...
                        <div className="bg-gradient-to-br from-brand-green/10 to-emerald-50 rounded-xl p-3.5 border border-brand-green/20">
                          <div className="flex items-start gap-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-text-muted leading-tight">Our AI will analyze your brand voice and create engaging content.</p>
                            </div>
                          </div>
                        </div>
                        <div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {currentStep > 0 && (
                <Button variant="brandOutline" onClick={prevStep} className="h-11 px-6">
                  <ArrowLeft className="w-4 h-4 mr-2" />
                  Back
                </Button>
              )}
              <Button variant="ghost" onClick={skipOnboarding} className="text-text-muted hover:text-text-primary h-11">
                Skip for now
              </Button>
            </div>
            <Button onClick={nextStep} variant="brand" className="h-11 px-8 shadow-lg shadow-brand-green/20">
